  ...
  AppModule = (function() {
    ...
    */
    ...
    function AppModule(filename, parent) {
      var componentInfo, fileContents;
      ...
      if (fs.existsSync(this.parent + "/component.json")) {
        fileContents = fs.readFileSync(this.parent + "/component.json");
        componentInfo = JSON.parse(fileContents);
        this.details = fileContents;
        this.componentInfo = componentInfo;
        if (componentInfo.name.length > 0) {
          this.id = componentInfo.namespace + "/" + componentInfo.name + "/" + this.id;
        } else {
          this.id = componentInfo.namespace + "/" + this.id;
        }
      }
    }
